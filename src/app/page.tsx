...
export default function Home() {
  ...
  const [modalType, setModalType] = useState<'contact' | 'rejoindre'>('contact');
  const [isSubmitting, setIsSubmitting] = useState(false);

  const openModal = (type: 'contact' | 'rejoindre') => {
    setModalType(type);
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const endpoint = modalType === 'contact' ? '/api/contact' : '/api/candidature';
      ...
      <main className="relative z-10 flex flex-col items-center justify-center min-h-[75vh] sm:min-h-[80vh] px-4 sm:px-6 text-center pt-8 sm:pt-0">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-base sm:text-xl md:text-2xl text-[#4fc3f7] mb-6 sm:mb-8 max-w-4xl mx-auto leading-relaxed px-2">
            Cloud computing éthique, accessible et éducatif à destination des jeunes. 
            Favoriser l&apos;autonomie numérique dans une logique de solidarité et d&apos;inclusion.
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center mb-8 sm:mb-12 px-4">
            ...
            >
              <span className="relative z-10 text-sm sm:text-base">Nous rejoindre</span>
          ...
          {/* Values */}
          <div id="values" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 mt-12 sm:mt-16 px-2">
            <div className="text-center p-4 sm:p-6 bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 hover:border-[#4fc3f7]/30 transition-all duration-300 hover:transform hover:scale-105">
              <div className="text-2xl sm:text-3xl font-bold text-[#4fc3f7] mb-2 sm:mb-3">🎓</div>
              <h3 className="text-base sm:text-lg font-semibold text-[#4fc3f7] mb-1 sm:mb-2">Éducatif</h3>
              <p className="text-xs sm:text-sm text-[#4fc3f7]/70 leading-relaxed">Apprentissage et autonomie numérique pour tous</p>
            </div>
            ...
            <div className="text-center p-4 sm:p-6 bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 hover:border-[#f97316]/30 transition-all duration-300 hover:transform hover:scale-105">
              <div className="text-2xl sm:text-3xl font-bold text-[#f97316] mb-2 sm:mb-3">🤝</div>
              <h3 className="text-base sm:text-lg font-semibold text-[#4fc3f7] mb-1 sm:mb-2">Solidaire</h3>
              <p className="text-xs sm:text-sm text-[#4fc3f7]/70 leading-relaxed">Accessible et inclusif dans une démarche équitable</p>
            </div>
            ...
            <div className="text-center p-4 sm:p-6 bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 hover:border-[#4fc3f7]/30 transition-all duration-300 hover:transform hover:scale-105 sm:col-span-2 lg:col-span-1">
              <div className="text-2xl sm:text-3xl font-bold text-[#4fc3f7] mb-2 sm:mb-3">🔒</div>
              <h3 className="text-base sm:text-lg font-semibold text-[#4fc3f7] mb-1 sm:mb-2">Éthique</h3>
              <p className="text-xs sm:text-sm text-[#4fc3f7]/70 leading-relaxed">Respect absolu de la vie privée et des données</p>
            </div>
          </div>
        </div>
      </main>
          ...
          <div className="bg-gradient-to-br from-[#00132c]/95 to-[#003F92]/95 backdrop-blur-md border border-white/20 rounded-2xl p-6 sm:p-8 max-w-md sm:max-w-lg w-full relative shadow-2xl animate-in fade-in duration-300">
            ...
            <div className="mb-6">
              <h2 className="text-xl sm:text-2xl font-bold text-[#4fc3f7] mb-2">
                {modalType === 'contact' ? 'Nous contacter' : 'Rejoindre Renardis'}
              </h2>
              <p className="text-[#4fc3f7]/70 text-sm">
                {modalType === 'contact' 
                  ? 'Une question ? Un projet ? Envie d&apos;en savoir plus ? Écrivez-nous !' 
                  : 'Envie de participer à cette aventure éthique ? Rejoignez-nous !'}
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {modalType === 'rejoindre' && (
                <>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[#4fc3f7] mb-2">
                      Compétences / Domaine d&apos;intérêt
                    </label>
                    <select 
                      name="competences"
                      disabled={isSubmitting}
                      className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-[#4fc3f7] focus:outline-none focus:border-[#4fc3f7] focus:ring-2 focus:ring-[#4fc3f7]/20 transition-all duration-300 cursor-pointer disabled:opacity-50"
                    >
                      <option value="">Sélectionnez un domaine</option>
                      <option value="dev">Développement</option>
                      <option value="design">Design / UX</option>
                      <option value="admin">Administration système</option>
                      <option value="marketing">Communication / Marketing</option>
                      <option value="juridique">Juridique / Administratif</option>
                      <option value="education">Éducation / Pédagogie</option>
              ...
              >
                {isSubmitting 
                  ? 'Envoi en cours...' 
                  : modalType === 'contact' ? 'Envoyer le message' : 'Candidater'}
              </button>
            </form>

            <p className="text-xs text-[#4fc3f7]/60 mt-4 text-center leading-relaxed">
              Vos données sont traitées dans le respect de votre vie privée et des valeurs éthiques de Renardis.
            </p>
          </div>
        </div>
      )}
